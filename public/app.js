...
const destinationSpan = document.getElementById('destination_station');
    ...
    async function populateDropdowns() {
        try {
            const response = await fetch('dataset/metro_coordinates.csv');
            if (!response.ok) {
    ...
    async function loadAndDisplayMap(showConnections = true) { // Added showConnections parameter
        ...
        try {
            const coordinatesResponse = await fetch('dataset/metro_coordinates.csv');
            ...
            if (showConnections) { // Only draw connections if showConnections is true
                ...
                const linesResponse = await fetch('dataset/Delhi_Metro_Lines.csv');
                if (!linesResponse.ok) {
    ...
    fetch('dataset/Delhi_Metro_Lines.csv')
    ...
    shortestPathBtn.addEventListener('click', async () => {
        ...
        if (source && destination && source !== destination) {
            try {
                const response = await fetch(`/shortest_path?source=${encodeURIComponent(source)}&destination=${encodeURIComponent(destination)}`);
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
                ...
                if (data.error) {
                    ...
                    const stationsData = await fetch('dataset/metro_coordinates.csv') // Fetch station coordinates again
                        .then(response => response.text())
                        .then(csvData => {
                            const lines = csvData.trim().split('\n').slice(1);
    ...
    minInterchangeBtn.addEventListener('click', async () => {
        ...
        if (source && destination && source !== destination) {
            try {
                const response = await fetch(`/min_exchanges?source=${encodeURIComponent(source)}&destination=${encodeURIComponent(destination)}`);
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
                const data = await response.json();
